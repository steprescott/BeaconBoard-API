...
namespace BB.BusinessLogicEntityFramework.Logic
{
    public class RoleBusinessLogic : IRoleBusinessLogic
    {
        ...
        public RoleBusinessLogic(IUnitOfWork unitOfWork)
        {
        ...
        {
            return _unitOfWork.GetAll<Role>().Any(i => i.RoleID == id);
        }

        public CRUDResult Create(Domain.Role domainObject)
        {
            try
            {
                //Check to see if the ID has been set on the domain object already
                if (domainObject.RoleID == Guid.Empty)
                {
                    //If it hasn't been set generate a new GUID
                    domainObject.RoleID = Guid.NewGuid();
                }

                //Map the domain object to an Entity Framework object
                var obj = Mapper.Map<Role>(domainObject);
        ...
        public List<Domain.Role> GetAll()
        {
            ...
            var items = _unitOfWork.GetAll<Role>().ToList();

            //Map all the Entity Framework items to a list of domain objects
            return Mapper.Map<List<Domain.Role>>(items);
        }

        public Domain.Role GetByID(Guid id)
        {
            //Get all the Entity Framework item with the given ID from the database
            var obj = _unitOfWork.GetById<Role>(id);

            //Map the Entity Framework item to a domain object
            return Mapper.Map<Domain.Role>(obj);
        }

        public CRUDResult Delete(Domain.Role domainObject)
        {
            //Use the ID of the domain object to call the DeleteByID function
            return DeleteByID(domainObject.RoleID);
        }
